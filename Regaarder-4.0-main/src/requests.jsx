...
const BoostsModal = ({ isOpen, onClose, requestId, detailedRank, onGiveLikeFree, selectedLanguage = 'English' }) => {
    ...
    const PAYMENT_LIKES_PER_USD = 10;
    const paymentLikes = selectedAmount * PAYMENT_LIKES_PER_USD;
    ...
    // Focus management: trap focus inside the modal while open and restore focus on close
    ...
    return (
        <div
            className={`fixed inset-0 z-50 transition-all duration-300 ${isOpen ? 'translate-y-0' : 'translate-y-full pointer-events-none'}`}
            style={{ transitionProperty: 'transform', transitionTimingFunction: 'cubic-bezier(.4,0,.2,1)' }}
            aria-hidden={!isOpen}
        >
            <div
            ...
            <div
                        ...
                        <div className="grid grid-cols-4 gap-3">
                            {[5, 10, 25, 50].map(amount => (
                                <button
                                    ...
                                    className="relative py-8 text-center rounded-2xl border-2 font-bold transition-all"
                                    style={{
                                        ...
                                        boxShadow: selectedAmount === amount ? '0 0 0 0 rgba(17, 24, 39, 0.7)' : 'none',
                                        animation: selectedAmount === amount ? 'pulse-border 2s infinite' : 'none',
                                    }}
                                >
                                    <div className="text-3xl mb-1">${amount}</div>
                                    <div className="text-xs opacity-60">+{amount * 2}</div>
                                </button>
                            ))}
                        </div>
                    </div>
                            ...
                            {topContributors.slice(0, 3).map((contributor, idx) => (
                                <div key={idx} className="flex items-center justify-between">
                                    <div className="flex items-center space-x-4">
                                        ...
                                        <div className="text-base text-gray-900">{contributor.name}</div>
                                    </div>
                                    <div className="text-base text-gray-400">${contributor.amount}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <footer className="px-8 pb-10 pt-6 border-t border-gray-50" style={{ paddingBottom: 'calc(40px + env(safe-area-inset-bottom))' }}>
                    <div className="space-y-4">
                        {/* Primary CTA - Bold and Clean */}
                        <button
                            className={`w-full py-5 font-semibold text-lg rounded-2xl transition-all ${processingPayment ? 'opacity-50 pointer-events-none' : 'hover:scale-[1.01] active:scale-[0.99]'}`}
                            style={{
